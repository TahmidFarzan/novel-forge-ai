...
import ModelPagination from '@/components/pagination/Pagination.vue'
...
import { canCreateGenre, canUpdateGenre, canDeleteGenre } from '@/composables/useUserPermissions'
...
const applyFilter = () => {
    ...
    intertiaJsRoute.get(route('genres.index'), cleanParams, {
        replace: true,
        preserveScroll: true,
        preserveState: true,
...
const canCreate = () => canCreateGenre(authUser?.value)
const canUpdate = (genre) => canUpdateGenre(authUser?.value, genre)
const canDelete = (genre) => canDeleteGenre(authUser?.value, genre)

const handleDelete = (genre) => {
    if (!genre || deleteProcessing.value) return

    deleteProcessing.value = true

    intertiaJsRoute.delete(route('genres.delete', { slug: genre?.slug }), {
        onFinish: () => {
            closeDeleteModal()
...
<template>
    ...
    <div class="w-full space-y-6">
        ...
        <div class="flex justify-between items-center">
            ...
            <a v-if="canCreate()" :href="route('genres.create')"
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full text-sm">
                    ...
                    <tbody class="divide-y">
                        <tr v-for="(item, index) in genres?.data" :key="item.id" class="hover:bg-gray-50 transition">
                            ...
                            <td class="px-4 py-3 text-right">
                                <div class="flex justify-end gap-2">
                                    ...
                                    <a :href="route('genres.details', { slug: item.slug })"
                                        class="p-2 rounded-md text-blue-600 hover:bg-blue-50 border"
                                        title="View Details">
                                        <FontAwesomeIcon icon="info" />
                                    </a>

                                    <a v-if="canUpdate(item)" :href="route('genres.edit', { slug: item.slug })"
