...
const applyFilter = () => {
    ...
    inertiaJsRouter.get(route('medias.index'), cleanParams, {
        replace: true,
        preserveScroll: true,
        preserveState: true,
...
const handleDelete = (media) => {
    ...
    inertiaJsRouter.delete(route('medias.delete', { slug: media?.slug }), {
        onFinish: () => {
    ...
    <div class="w-full space-y-6">
        ...
        <form @submit.prevent="applyFilter" class="bg-white border border-gray-200 rounded-xl shadow-sm p-5 space-y-4">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <input type="search" v-model="filterForm.search" placeholder="Enter search"
                    class="w-full border border-gray-200 rounded-md px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none" />
            ...
            <div class="flex justify-end">
                <button type="submit" :disabled="filterForm.processing"
                    ...
                    {{ filterForm.processing ? 'Applying Filter' : 'Apply Filter' }}
                </button>
            </div>
        </form>
                    ...
                    <tbody class="divide-y">
                        <tr v-for="(item, index) in medias?.data" :key="item.id" class="hover:bg-gray-50 transition">
                            ...
                            <td class="px-4 py-3 text-gray-600">
                                {{ item.collection_name ?? 'Not Available' }}
                            </td>

                            <td class="px-4 py-3 text-gray-500">
                                {{ item.created_at ? formatDateTime(item.created_at) : 'Not Available' }}
                            </td>

                            <td class="px-4 py-3 text-right">
                                <div class="flex justify-end gap-2">

                                    <a :href="route('medias.details', { slug: item.slug })"
