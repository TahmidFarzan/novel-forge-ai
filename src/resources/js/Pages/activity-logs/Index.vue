...
const applyFilter = () => {
    ...
    inertiaJsRoute.get(route('activity-logs.index'), cleanParams, {
        replace: true,
        preserveScroll: true,
        preserveState: true,
    })
...
const handleDelete = (activityLog) => {
    ...
    inertiaJsRoute.delete(route('activity-logs.delete', { slug: activityLog?.slug }), {
        onFinish: () => {
        ...
        <form @submit.prevent="applyFilter" class="bg-white border border-gray-200 rounded-xl shadow-sm p-5 space-y-4">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <input v-model="filterForm.search" type="search"
                    placeholder="Search by log name, event, or description..."
                        ...
                        <tr v-for="(item, index) in activityLogs?.data" :key="item.id"
                            ...
                            <td class="px-4 py-3 text-right">
                                <div class="flex justify-end gap-2">
                                    ...
                                    <a :href="route('activity-logs.details', { slug: item.slug })"
