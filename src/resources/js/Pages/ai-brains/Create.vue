...
function validateForm() {
    ...
    if (!saveForm.context_window) {
        saveForm.setError("context_window", "Context window is required");
        valid = false;
    }
...
function handleSave() {
    ...
    if (isUpdate.value) {
        intertiaJsRoute.post(
            route("ai-brains.update", { slug: aiBrain?.slug }),
            { ...saveForm.data(), _method: "patch" },
            requestConfig,
        );
    } else {
        saveForm.post(route("ai-brains.save"), requestConfig);
    }
}
