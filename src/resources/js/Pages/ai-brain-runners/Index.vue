...
const { aiBrainRunners } = defineProps({
    aiBrainRunners: Object,
})

const paginationOnly = computed(() => {
    if (!aiBrainRunners) return {}
    const { data, ...rest } = aiBrainRunners
    return rest
})
...
const applyFilter = () => {
    ...
    intertiaJsRoute.get(route('ai-brains.index'), cleanParams, {
        replace: true,
        preserveScroll: true,
        preserveState: true,
...
onMounted(async () => {
    ...
    window.dispatchEvent(
        new CustomEvent('set-breadcrumb', {
            detail: [
                { text: 'Ai Brain Runner', active: true },
            ],
        })
    )
})
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full text-sm">
                    ...
                    <tbody class="divide-y">
                        <tr v-for="(item, index) in aiBrainRunners?.data" :key="item.id" class="hover:bg-gray-50 transition">
                            ...
                            <td class="px-4 py-3 text-right">
                                <div class="flex justify-end gap-2">
                                    ...
                                    <a :href="route('ai-brain-runners.details', { slug: item.slug })"
                        ...
                        <tr v-if="!aiBrainRunners?.data?.length">
                            <td colspan="5" class="px-4 py-8 text-center text-gray-500">
                                No ai brain runners found
                            </td>
                        </tr>
                    </tbody>
