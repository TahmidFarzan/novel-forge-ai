...
import ApiGroupedSelect from '@/components/multi-select/ApiGroupedSelect.vue'
...
function validateForm() {
    ...
    if (!saveForm.user_permission_ids && !saveForm?.is_super_admin) {
        saveForm.setError('user_permission_ids', 'Please select at least one permission')
        valid = false
    }
...
function handleSave() {
    ...
    if (isUpdate.value) {
        intertiaJsRoute.post(
            route('users.update', { slug: user?.slug }),
            { ...saveForm.data(), _method: 'patch' },
            requestConfig
        )
    } else {
        saveForm.post(route('users.save'), requestConfig)
    }
}
...
onMounted(async () => {
    ...
    window.dispatchEvent(
        new CustomEvent('set-breadcrumb', {
            detail: [
                { text: 'Users', href: route('users.index') },
            ...
            <form @submit.prevent="handleSave" class="space-y-6 mt-3">
                <ul class="list-disc pl-5">
                    <li class="text-base font-semibold text-blue-700">
                        Note: Super admin users have full system access and can manage all features.
                    </li>
                </ul>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <InfiniteScrollApiSelect :form="saveForm" fieldName="marital_status"
                                ...
                                :multiple="false" placeholder="Select marital status"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="md:col-span-2 flex gap-6 items-center pt-2">
                            ...
                            <label class="flex items-center gap-2">
                                ...
                                <span class="text-sm">Send verification email</span>
                            </label>
