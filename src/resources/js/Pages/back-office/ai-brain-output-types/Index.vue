...
const { aiBrainOutputTypes } = defineProps({
    aiBrainOutputTypes: Object,
})

const paginationOnly = computed(() => {
    if (!aiBrainOutputTypes) return {}
    const { data, ...rest } = aiBrainOutputTypes
    return rest
})
...
const applyFilter = () => {
    ...
    intertiaJsRoute.get(route('back-office.ai-brain-output-types.index'), cleanParams, {
        replace: true,
        preserveScroll: true,
        preserveState: true,
...
onMounted(async () => {
    ...
    window.dispatchEvent(
        new CustomEvent('set-breadcrumb', {
            detail: [
                { text: 'Ai Brain Output Types', active: true },
            ],
        })
    )
})
...
<template>
    <Head :title="'Ai Brain Output Types'" />
                ...
                <table class="min-w-full text-sm">
                    ...
                    <thead class="bg-gray-50 text-gray-600 text-xs uppercase">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-left">Brief</th>
                    ...
                    <tbody class="divide-y">
                        <tr v-for="(item, index) in aiBrainOutputTypes?.data" :key="item.id"
                            ...
                            <td class="px-4 py-3 text-right">
                                <div class="flex justify-end gap-2">
                                    ...
                                    <a :href="route('back-office.ai-brain-output-types.details', { slug: item.slug })"
                        ...
                        <tr v-if="!aiBrainOutputTypes?.data?.length">
                            <td colspan="6" class="px-4 py-8 text-center text-gray-500">
                                No ai brain output types found
                            </td>
                        </tr>
                    </tbody>
