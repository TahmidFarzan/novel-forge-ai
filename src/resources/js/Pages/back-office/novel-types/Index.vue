...
import { canCreateNovelType, canUpdateNovelType, canDeleteNovelType } from '@/composables/useUserPermissions'
...
const { novelTypes } = defineProps({
    novelTypes: Object,
})

const paginationOnly = computed(() => {
    if (!novelTypes) return {}
    const { data, ...rest } = novelTypes
    return rest
})
...
const applyFilter = () => {
    ...
    intertiaJsRoute.get(route('back-office.novel-types.index'), cleanParams, {
        replace: true,
        preserveScroll: true,
        preserveState: true,
...
const confirmDelete = (novelType) => {
    deletingRow.value = novelType
...
const canCreate = () => canCreateNovelType(authUser?.value)
...
const handleDelete = (novelType) => {
    if (!novelType || deleteProcessing.value) return
...
<template>
    ...
    <div class="w-full space-y-6">
        ...
        <div class="flex justify-between items-center">
            ...
            <a v-if="canCreate()" :href="route('back-office.novel-types.create')"
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full text-sm">
                    ...
                    <tbody class="divide-y">
                        <tr v-for="(item, index) in novelTypes?.data" :key="item.id" class="hover:bg-gray-50 transition">
                            ...
                            <td class="px-4 py-3 text-right">
                                <div class="flex justify-end gap-2">
                                    ...
                                    <a :href="route('back-office.novel-types.details', { slug: item.slug })"
                                        class="p-2 rounded-md text-blue-600 hover:bg-blue-50 border"
                                        title="View Details">
                                        <FontAwesomeIcon icon="info" />
                                    </a>

                                    <a v-if="canUpdate(item)" :href="route('back-office.novel-types.edit', { slug: item.slug })"
                        ...
                        <tr v-if="!novelTypes?.data?.length">
                            <td colspan="5" class="px-4 py-8 text-center text-gray-500">
                                No novel types found
                            </td>
                        </tr>
                    </tbody>
