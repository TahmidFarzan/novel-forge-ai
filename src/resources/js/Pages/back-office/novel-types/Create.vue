...
const isUpdate = computed(() => !!novelType?.slug)
...
function handleSave() {
    ...
    if (isUpdate.value) {
        intertiaJsRoute.post(
            route('back-office.novel-types.update', { slug: novelType?.slug }),
            { ...saveForm.data(), _method: 'patch' },
            requestConfig
        )
    } else {
        saveForm.post(route('back-office.novel-types.save'), requestConfig)
    }
}
                ...
                <div class="bg-white border rounded-xl p-5 shadow-sm space-y-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input v-model="saveForm.name" placeholder="Enter novel type name"
                ...
                <div class="bg-white border rounded-xl p-5 shadow-sm space-y-4">
                    ...
                    <div>
                        ...
                        <textarea v-model="saveForm.prompt_instruction" rows="8"
                            placeholder="Enter the master instruction for AI form generation for this novel type. This controls story length, narrative scope, structure, development depth and page requirements."
                            class="w-full border rounded-md px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none"
