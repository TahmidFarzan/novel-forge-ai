...
FontAwesomeLibrary.add(faFileLines);
...
function submit() {
    if (pagePlansForm.processing) {
        return;
    }

    pagePlansForm.clearErrors();

    if (!pagePlansForm.ai_brain_id) {
        pagePlansForm.setError(
            "ai_brain_id",
        ...
        >
            ...
            >
                <InfiniteScrollApiSelect
                    ...
                    :selectedItem="pagePlansForm.ai_brain_id"
                    :apiUrl="buildAiBrainSearchUrl()"
                    :multiple="false"
                    placeholder="Select AI Brain"
                    :error="pagePlansForm.errors.ai_brain_id"
                    class="ai-brain-select"
                />
            </div>

            <p
                v-if="pagePlansForm.errors.ai_brain_id"
                class="text-red-500 text-sm"
            >
                {{ pagePlansForm.errors.ai_brain_id }}
            </p>
        </div>
    </div>
</template>
