...
const ownIndex = computed(() => {
    const foundIndex = groupImages.value.findIndex(
        (item) => item.uid === uid
    )
...
const activeImage = computed(() => {
    return groupImages.value?.[activeIndex.value] ||
        normalizedImage.value ||
        null
})
...
<template>
    <figure class="light-box-container" :class="figureClass">
        ...
        <div
            v-if="shouldShowItemCounter"
            class="flex items-center gap-2 text-sm leading-6 text-gray-600"
        >
        ...
        >
            ...
            <div class="absolute inset-0 flex items-center justify-center rounded-2xl bg-black/0 opacity-0 transition duration-300 group-hover:bg-black/35 group-hover:opacity-100">
                <div class="flex h-14 w-14 scale-90 items-center justify-center rounded-full bg-white/90 text-xl text-gray-900 shadow-lg transition duration-300 group-hover:scale-100">
        ...
        <Teleport to="body">
            <Transition name="lightbox-fade">
                <div
                    v-if="isOpen"
                    class="fixed inset-0 z-[9999] flex bg-black/95 text-white"
                    role="dialog"
                    aria-modal="true"
                >
                    <button
                        ...
                        >
                            ...
                            <Transition name="image-slide" mode="out-in">
                                <img
                                    ...
                                    :src="activeImage.src"
                                    :alt="activeImage.alt"
                                    class="select-none object-contain transition-transform duration-300"
                                    :class="isZoomed
