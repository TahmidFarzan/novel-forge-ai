...
const { media, mediaClass } = defineProps({
    ...
    mediaClass: {
        type: String,
        default:
            "w-full h-full object-contain rounded-lg border border-gray-200 shadow-sm",
    },
});
</script>
    ...
    <div>
        ...
        >
            <img
                ...
                :srcset="media?.media_srcset"
                loading="lazy"
                :class="mediaClass"
            />

            <figcaption
                v-if="media?.custom_properties?.caption"
        ...
        <iframe
            v-else-if="
                ['application/json', 'text/plain', 'text/csv'].includes(
                    media?.mime_type,
                )
            "
            :src="media?.preview_url || media?.original_url"
        ...
        <div
            v-else-if="
                ['application/zip', 'application/x-rar-compressed'].includes(
                    media?.mime_type,
                )
            "
        >
            <a
                :href="media?.preview_url || media?.original_url"
                download
